...
async def fast_batch_forward(user, bot, msg_ids, m, sts, protect, batch_size=200):
    ...
    async def forward_one(mid):
        try:
            ...
            print(f"Error forwarding {mid}: {ex}")
            sts.add('deleted')
...
# ... (the rest of your file remains unchanged) ...       await client.get_messages(sts.get("FROM"), sts.get("limit"))
    ...
    if locked:
        try:
          pling=0
          batch_size = 50  # TUNE THIS: 50-100 for best speed!
          MSG = []
          await edit(user, m, 'ᴘʀᴏɢʀᴇssɪɴɢ', 5, sts)
          async for message in iter_messages(client, chat_id=sts.get("FROM"), limit=sts.get("limit"), offset=sts.get("skip"), filters=filter, max_size=max_size):
                if await is_cancelled(client, user, m, sts):
...
async def restart_pending_forwads(bot, user):
    ...
    try:
       ...
       try: 
          ...
          await msg_edit(m, f"**Source chat may be a private channel / group. Use userbot (user must be member over there) or  if Make Your [Bot](t.me/{_bot['username']}) an admin over there**", retry_btn(forward_id))
          return await stop(client, user)
       try:
          k = await client.send_message(i.TO, "Testing")
          await k.delete()
...
async def complete_time(total_files, files_per_minute=30):
    ...
    return time_format                if await is_cancelled(client, user, m, sts):
                   if user_have_db:
                      await user_db.drop_all()
                      await user_db.close()
                   return
